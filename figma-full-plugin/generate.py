...
T = {
    "page": "Full UI \u00b7 Splash + Hi-Fi",
    "notify": "\u5df2\u751f\u6210\uff1a\u5f00\u573a\u5173\u952e\u5e27 + \u9ad8\u4fdd\u771f\uff08\u542b Logo\uff09",
    "fail": "\u751f\u6210\u5931\u8d25: ",
    "cover": "\u89e3\u4e09\u89d2\u5f62 \u00b7 \u5b8c\u6574\u5305\uff08\u5f00\u573a + \u9ad8\u4fdd\u771f\uff09",
    "cover_sub": (
        "\u542b App Logo \u5f00\u573a\u5173\u952e\u5e27\uff08\u53ef Smart Animate\uff09 + 5 \u4e2a\u9ad8\u4fdd\u771f\u5c4f\u5e55 "
        "\u00b7 \u65b0\u9875\u9762\uff0c\u4e0d\u6539\u539f\u6587\u4ef6"
    ),
    "splash_brand": "\u89e3\u4e09\u89d2\u5f62",
    "splash_tag": "AI \u5e26\u4f60\u4e00\u6b65\u4e00\u6b65\u89e3\u9898",
    "splash_tip": "Prototype: Splash/01 \u2192 02 \u2192 03 \u2192 04 \u2192 01 Home \u00b7 Smart Animate",
    "s01": "01 Home \u00b7 \u9996\u9875",
    "s02": "02 Search \u00b7 \u641c\u7d22\u9898\u76ee",
    "s03": "03 Guide \u00b7 AI \u5f15\u5bfc",
    "s04": "04 Solid \u00b7 \u7acb\u4f53",
...
def main() -> None:
    ...
    logo_path = Path(__file__).with_name("logo.b64")
    if not logo_path.exists():
        raise SystemExit("Missing logo.b64 — run README regen steps first")
    logo_b64 = logo_path.read_text(encoding="ascii").strip()

    out = f'''/**
 * Triangle Guide Full — Hi-Fi + Splash + Logo
...
function ensureLogo() {{
  if (LOGO_HASH) return LOGO_HASH;
  const bytes = figma.base64Decode(LOGO_B64);
  const image = figma.createImage(bytes);
  LOGO_HASH = image.hash;
  return LOGO_HASH;
}}

function placeLogo(parent, x, y, size) {{
  const hash = ensureLogo();
  const r = figma.createRectangle();
  r.name = "app-logo";
  r.x = x;
  r.y = y;
  r.resize(size, size);
  r.fills = [{{ type: "IMAGE", scaleMode: "FIT", imageHash: hash }}];
  r.strokes = [];
...
async function buildCover(page) {{
  const c = frame("00 Cover", 0, 0, 1180, 220, {{
    parent: page,
    fills: solid(C.white),
    stroke: C.line,
    strokeWeight: 1,
  ...
  placeLogo(c, 36, 36, 72);
  await txt(c, "t", {j(T["cover"])}, 128, 40, 26, {{ bold: true, w: 900 }});
  await txt(c, "s", {j(T["cover_sub"])}, 128, 82, 13, {{ color: C.muted, w: 980 }});
  await txt(c, "tip", {j(T["splash_tip"])}, 128, 150, 12, {{ color: C.navy, w: 980 }});
  const swatches = [
    ["navy", C.navy],
    ["orange", C.orange],
    ["softBlue", C.softBlue],
  ...
  for (let i = 0; i < swatches.length; i++) {{
    rect(c, swatches[i][0], 36 + i * 56, 170, 40, 32, {{
...
async function splashFrame(page, name, x, y, opts) {{
  opts = opts || {{}};
  const f = frame(name, x, y, 390, 720, {{
    parent: page,
    fills: solid(C.splashBg),
    stroke: C.line,
    strokeWeight: 1,
    radius: 28,
  }});
  // soft glows
  const g1 = figma.createEllipse();
  g1.resize(220, 160);
  g1.x = -40;
  g1.y = -20;
  g1.fills = solid(C.softBlue, 0.55);
  f.appendChild(g1);
  const g2 = figma.createEllipse();
  g2.resize(200, 160);
  g2.x = 240;
  g2.y = 520;
  g2.fills = solid(C.orange, 0.12);
  f.appendChild(g2);

  const logoSize = opts.logoSize || 0;
  const logoY = opts.logoY == null ? 160 : opts.logoY;
  if (logoSize > 0) {{
    placeLogo(f, (390 - logoSize) / 2, logoY, logoSize);
  }}
  if (opts.symbols) {{
    await txt(f, "sym+", "+", 48, 120, 28, {{ color: C.pink, bold: true }});
    await txt(f, "symdiv", "\\u00f7", 180, 88, 30, {{ color: C.yellow, bold: true }});
    await txt(f, "symsqrt", "\\u221a", 300, 122, 28, {{ color: C.sky, bold: true }});
  }}
  if (opts.brand) {{
    await txt(f, "brand", {j(T["splash_brand"])}, 95, 470, 28, {{ color: C.navy, bold: true, w: 220 }});
    await txt(f, "tag", {j(T["splash_tag"])}, 70, 512, 14, {{ color: C.muted, w: 260 }});
  }}
  if (opts.bar) {{
    rect(f, "bar-bg", 135, 560, 120, 4, {{ fills: solid(C.line), radius: 999 }});
    rect(f, "bar-fg", 135, 560, opts.bar, 4, {{ fills: solid(C.orange), radius: 999 }});
  }}
  await txt(f, "frame-label", name, 16, 16, 11, {{ color: C.muted, bold: true, w: 280 }});
  return f;
}}

async function buildSplash(page, ox, oy) {{
  await txt(page, "label-splash", "Splash \\u00b7 \u5f00\u573a\u5173\u952e\u5e27\uff08Smart Animate\uff09", ox, oy - 28, 14, {{ bold: true }});
  // 01 empty / logo tiny
  await splashFrame(page, "Splash / 01", ox, oy, {{ logoSize: 120, logoY: 220 }});
  // 02 logo pop
  await splashFrame(page, "Splash / 02", ox + 420, oy, {{ logoSize: 240, logoY: 150 }});
  // 03 symbols
  await splashFrame(page, "Splash / 03", ox + 840, oy, {{ logoSize: 240, logoY: 150, symbols: true }});
  // 04 brand + progress full
  await splashFrame(page, "Splash / 04", ox + 1260, oy, {{
    logoSize: 220,
    logoY: 130,
    symbols: true,
    brand: true,
...
async function main() {{
  ...
  await buildSplash(page, 0, 300);
  await buildHome(page, 0, 1140);
  await buildSearch(page, 0, 1980);
  await buildGuide(page, 0, 2840);
  await buildSolid(page, 0, 3700);
  await buildDone(page, 0, 4560);
...
'''
    ...
    assert "LOGO_B64" in text
    assert "Splash / 01" in text
    assert len(logo_b64) > 1000
    print("OK", dest, dest.stat().st_size, "bytes", "logo_b64", len(logo_b64))
